...
public interface AccountService extends PagingAndSortingRepository<Account, Long>, JpaSpecificationExecutor<Account> {
    ...
    List<Account> findByCourseMasterAndRegisterDateBetween(Master master, @Temporal(TemporalType.TIMESTAMP) Date startDate, @Temporal(TemporalType
            .TIMESTAMP) Date endDate);

    List<Account> findByCourseMasterInAndRegisterDateBetween(List<Master> masters, @Temporal(TemporalType.TIMESTAMP) Date startDate, @Temporal
            (TemporalType.TIMESTAMP) Date endDate);

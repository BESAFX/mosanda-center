...
public interface BankTransactionService extends PagingAndSortingRepository<BankTransaction, Long>, JpaSpecificationExecutor<BankTransaction> {
    ...
    <T> List<T> findByDateBetweenOrTransactionTypeCodeIn(@Temporal(TemporalType.TIMESTAMP) Date startDate, @Temporal(TemporalType.TIMESTAMP) Date
            endDate, List<String> transactionTypeCodes, Class<T> type);

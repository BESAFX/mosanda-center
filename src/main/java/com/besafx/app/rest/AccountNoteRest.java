...
public class AccountNoteRest {
    ...
    private final String FILTER_TABLE = "**,account[id],person[id,contact[id,shortName]]";
    ...
    public String create(@RequestBody AccountNote accountNote) {
        Person caller = ((PersonAwareUserDetails) SecurityContextHolder.getContext().getAuthentication().getPrincipal()).getPerson();
        accountNote.setPerson(caller);
        accountNote.setDate(new DateTime().toDate());
        accountNote = accountNoteService.save(accountNote);
        notificationService.notifyAll(Notification.builder().message("تم حفظ ملاحظة جديدة بنجاح").type(NotificationDegree.success).build());

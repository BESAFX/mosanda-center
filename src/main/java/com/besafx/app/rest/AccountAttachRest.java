...
public class AccountAttachRest {
        ...
        Optional.ofNullable(files).ifPresent(value -> {
            ...
            while (fileListIterator.hasNext()) {
                ...
                if (result) {
                    ...
                    attach.setLink(link);
                    notificationService.notifyAll(Notification.builder().message("تم رفع الملف" + " [ " + file.getOriginalFilename() + " ] " + " بنجاح.").type(NotificationDegree.success).build());
    ...
    public String setType(
        ...
        if (accountAttach != null) {
            ...
            notificationService.notifyAll(Notification.builder().message("تم تعديل نوع الملف بنجاح").type(NotificationDegree.success).build());
    ...
    public String setName(
        ...
        if (accountAttach != null) {
            ...
            accountAttach = accountAttachService.save(accountAttach);
            notificationService.notifyAll(Notification.builder().message("تم تعديل اسم الملف بنجاح").type(NotificationDegree.success).build());
            return SquigglyUtils.stringify(Squiggly.init(new ObjectMapper(), FILTER_TABLE), accountAttach);
        } else {
            return null;
        }
    ...
    public Boolean delete(@PathVariable Long id) throws ExecutionException, InterruptedException {
        ...
        if (accountAttach != null) {
            ...
            if (deleteTask.get()) {
                ...
                notificationService.notifyAll(Notification.builder().message("لا يمكن حذف الملف" + " [ " + accountAttach.getAttach().getName() + " ] ").type(NotificationDegree.error).build());
                return false;
            }
        } else {
            return false;
    ...
    public void deleteWhatever(@PathVariable Long id) throws ExecutionException, InterruptedException {
        ...
        if (accountAttach != null) {
            accountAttachService.delete(accountAttach);
            notificationService.notifyAll(Notification.builder().message("تم حذف المرفق" + " [ " + accountAttach.getAttach().getName() + " ] " + " بنجاح.").type(NotificationDegree.success).build());
        }
    }
